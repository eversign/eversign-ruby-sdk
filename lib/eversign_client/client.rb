...
module EversignClient
	class Client
		...
		DOCUMENT_GET_PATH = '/document{?access_key,business_id,type}'
		DOCUMENT_CREATE_PATH = '/document{?access_key,business_id}'
		DOCUMENT_DELETE_PATH = '/document{?access_key,business_id,document_hash}'
		...
		def list_document(business_id, doc_type)
	  	template = Addressable::Template.new(self.base_uri + DOCUMENT_GET_PATH)
		...
		def create_document(business_id, document)
			template = Addressable::Template.new(self.base_uri + DOCUMENT_CREATE_PATH)
		...
		def delete_document(business_id, document_hash)
			template = Addressable::Template.new(self.base_uri + DOCUMENT_DELETE_PATH)
